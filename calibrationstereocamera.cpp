...
void CalibrationStereoCamera::findAndStoreChessBoardPoints() {
  ...
  while (itCalPicLeftCam != endItCalPicLeftCam &&
    ...
    // Left Camera
    bool patternFoundLeftCamera =
        findChessboardCorners(leftPicture, chessBoardSize, cornersLeftCamera);
    ...
    // Right Camera
    bool patternFoundRightCamera =
        findChessboardCorners(rightPicture, chessBoardSize, cornersRightCamera);
    //        CALIB_CB_ADAPTIVE_THRESH + CALIB_CB_NORMALIZE_IMAGE +
    //            CALIB_CB_FAST_CHECK);
...
void CalibrationStereoCamera::calibrateStereoCamera() {
  ...
  float error = stereoCalibrate(
      ...
      distL, new_mtxR, distR, leftPicture.size(), Rot, Trns, Emat, Fmat,
  ...
  stereoRectify(new_mtxL, distL, new_mtxR, distR, rightPicture.size(), Rot,
                Trns, rect_l, rect_r, proj_mat_l, proj_mat_r, Q,
                cv::CALIB_ZERO_DISPARITY, -1, rightPicture.size(), &RoiLeft,
                &RoiRight);
}

void CalibrationStereoCamera::UndistortRectifyMap() {
  // left Camera
  initUndistortRectifyMap(new_mtxL, distL, rect_l, proj_mat_l,
                          leftPicture.size(), CV_16SC2, leftStereoMapX,
                          leftStereoMapY);
  // right Camera
  initUndistortRectifyMap(new_mtxR, distR, rect_r, proj_mat_r,
                          rightPicture.size(), CV_16SC2, rightStereoMapX,
                          rightStereoMapY);
}

void CalibrationStereoCamera::saveFile() {
  FileStorage cvFile =
      FileStorage("calibrationParamaters.xml", FileStorage::WRITE);
  ...
  cvFile.write("Q", Q);
  cvFile.write("RoiLeftX", RoiLeft.x);
  cvFile.write("RoiLeftY", RoiLeft.y);
  cvFile.write("RoiLeftHeight", RoiLeft.height);
  cvFile.write("RoiLeftWidth", RoiLeft.width);
  cvFile.write("RoiRightX", RoiRight.x);
...
int CalibrationStereoCamera::getSquareSize() const { return squareSize; }

void CalibrationStereoCamera::setSquareSize(int value) { squareSize = value; }
