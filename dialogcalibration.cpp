...
DialogCalibration::DialogCalibration(QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogCalibration) {
  ui->setupUi(this);
  this->setWindowTitle("Calibration");
}

DialogCalibration::~DialogCalibration() { delete ui; }

void DialogCalibration::on_btnExit_clicked() { reject(); }

void DialogCalibration::on_btnStart_clicked() {
  CalibrationStereoCamera calibrationStereoCamera(
      this->pathCalibrationLeftCamera, this->pathCalibrationRightCamera);
